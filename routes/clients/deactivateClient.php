...
try {
    ...
    if (!in_array($loggedInUserRole, ['super_admin', 'admin'], true)) {
        throw new Exception("Unauthorized: Only Super Admins or Admins can deactivate clients", 403);
    }
    ...
    if (!isset($data['clientIds']) || !is_array($data['clientIds']) || count($data['clientIds']) === 0) {
        throw new Exception("Please select at least one client to deactivate.", 400);
    }
    ...
    try {
         ...
         */
        // Check for outstanding/active invoices
        $invoiceCheck = $conn->prepare("
            SELECT client_id FROM invoices 
            WHERE client_id IN ($placeholders) AND status IN ('sent', 'partial', 'overdue') 
            LIMIT 1
        ");
        $invoiceCheck->bind_param(str_repeat('i', count($clientIds)), ...$clientIds);
        $invoiceCheck->execute();
        
        if ($invoiceCheck->get_result()->num_rows > 0) {
            $invoiceCheck->close();
            throw new Exception(
                "Cannot deactivate. One or more selected clients have outstanding or unpaid invoices. Please resolve them first.", 
                409
            );
        }
        $invoiceCheck->close();
        ...
        if ($quoteCheck->get_result()->num_rows > 0) {
            $quoteCheck->close();
            throw new Exception(
                "Cannot deactivate. One or more selected clients have active Quotations. Please reject or convert them first.", 
                409
            );
        }
        $quoteCheck->close();
         ...
         */
        $updateQuery = "UPDATE clients SET is_active = 0 WHERE id IN ($placeholders) AND is_active = 1";
        ...
        if (!$updateStmt->execute()) {
            throw new Exception("Failed to deactivate clients: " . $updateStmt->error, 500);
        }

        if ($updateStmt->affected_rows === 0) {
            throw new Exception("Selected clients are already deactivated or do not exist.", 404);
        }
        ...
        $description = "{$loggedInUserEmail} deactivated client account(s) with ID(s): " . implode(', ', $clientIds);
        ...
        echo json_encode([
            "status"  => "success",
            "message" => "Client account(s) deactivated successfully."
        ]);
    ...
    error_log("Deactivate Client Error: " . $e->getMessage());
